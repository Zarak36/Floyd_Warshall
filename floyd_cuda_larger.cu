...
void generateLargeMatrix(std::vector<std::vector<int>>& dist, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            if (i == j) {
                ...
                // Random weights between 1 and 100 or INF
                dist[i][j] = (rand() % 10 == 0) ? INF : (rand() % 100 + 1);
            }
        }
    }
}
...
int main() {
    int n = 2048; // Number of vertices in the graph (example)
    std::vector<std::vector<int>> dist = {n, std::vector<int>(n)};

    //generate a random matrix
    generateLargeMatrix(dist, n);
